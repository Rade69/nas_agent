...
| **Voice Communication Reliability** (stabilizacija glasovne komunikacije — single-flight connect, cleanup, transport health, pouzdan event transport, fail-safe tool lifecycle, kontrolisan reconnect, VAD profili) | ✅ **R0 završen i verifikovan (pi + Codex, 2026-07-13)** + uski live-connect hotfix (Codex, 2026-07-13). ✅ **R1 završen (pi + Codex review fix, 2026-07-14)**: single-flight `connect()`, connect timeout, AbortController cancel/disconnect, `connectionGeneration` guard, transport health, error klasifikacija, 197 testova. ✅ **R2 završen (pi + Codex review fix, 2026-07-14)**: kontrolisani reconnect (max 3 pokušaja, backoff 1s/2s/4s + jitter, samo za transport/network greške), `manualDisconnectRequested` flag razlikuje korisnički Stop od transport failure-a, outbound event queue (max 50 eventova, max age 10s, flush na DC open/reconnect success), queue se čuva kroz reconnect i može primati evente tokom reconnect delay-a, neuspjeli reconnect setup zakazuje sljedeći pokušaj do limita, DNS/network greške (`getaddrinfo failed`, `Errno 11001`, `ENOTFOUND`, `EAI_AGAIN`, `fetch failed`) mapiraju se u korisničku mrežnu poruku, `_shouldReconnect` policy helper, `_handleTransportFailure` ruter, reconnect status poruke ("Pokušavam ponovo 1/3…", "Ponovo povezano."), 8 novih R2 testova (205 ukupno). ✅ **R3 završen (pi + Codex review fix, 2026-07-14)**: `activeToolCalls` + `completedToolCallIds` tracking, `_runToolWithTimeout` (30s per-tool deadline) sa cleanup-om timeout timera, `_sendToolOutput` sa generation/manualDisconnect guard-om, duplicate `call_id` odbijanje bez duplog izvršavanja, active duplicate ne završava originalni poziv prerano, exception-safe output uključujući `TOOL_TIMEOUT`, `thumbnail_loading_prepare` je u istom fail-safe lifecycle-u, stale output poslije disconnect-a se ne šalje, batch nastavlja poslije failure-a, 11 novih R3 testova (216 ukupno). R4 (diagnostics panel, provider abstraction) ostaje sljedeći paket. | FAZA 8 (voice-first UI), FAZA 6 (realtime session security) | [VOICE_COMMUNICATION_RELIABILITY_IMPLEMENTATION_PLAN_FOR_PI.md](./VOICE_COMMUNICATION_RELIABILITY_IMPLEMENTATION_PLAN_FOR_PI.md), [VOICE_COMMUNICATION_RELIABILITY_ANALYSIS_2026-07-13.md](./VOICE_COMMUNICATION_RELIABILITY_ANALYSIS_2026-07-13.md), [VOICE_COMMUNICATION_R1_BRIEF_FOR_PI.md](./VOICE_COMMUNICATION_R1_BRIEF_FOR_PI.md), [VOICE_COMMUNICATION_R2_BRIEF_FOR_PI.md](./VOICE_COMMUNICATION_R2_BRIEF_FOR_PI.md), [VOICE_COMMUNICATION_R3_BRIEF_FOR_PI.md](./VOICE_COMMUNICATION_R3_BRIEF_FOR_PI.md) |

### Confirmation Bridge dopuna (2026-07-15)

Post-approval feedback gap je zatvoren: rezultat UI retryja nakon odobrenja sada se vraća aktivnoj Realtime sesiji, uključujući klik iz zasebnog Computer Mode mini prozora preko named IPC mosta do glavnog renderer-a. Agent dobija eksplicitnu instrukciju da potvrdi korisniku da je odobrenje primljeno i kaže da li je izvršenje uspjelo. Istim paketom je `agent_name` povezan sa idle GUI naslovom iznad mikrofona i osvježava se odmah nakon čuvanja u Settings. Vidi `agent_reports/2026-07-15_confirmation-feedback-and-agent-name-gui.md`.

### Precizno otvaranje browsera (2026-07-15)
...
- **C4** ✅ (pi, 2026-07-16): publishing dokumentacija (`BROWSER_BRIDGE_PUBLISHING.md`), protocol versioning (v2), rate limiting (5/min), credential storage audit, privacy hardening (manifest), 5 novih error kodova, 2 nova testa. **Ukupno: 59 testova.**
...
| QM-2 | Companion orb integracija | ✅ urađeno (kod: `desktop/ui/orb.py` RickyOrbWidget portovan iz spike-a, `orb_window.py` kontekst meni lokalizovan + position lock, `voice_state.py` VoiceState→orb-state mapiranje; 15 testova. **QM-3 ispravka:** voice-state izvor je lokalni `VoiceStateBus` (Qt signal), ne HTTP polling na `GET /voice/state` — glas živi u Qt procesu, backend roundtrip je bespotreban). **Čeka:** stvarni VoiceState iz glasa (QM-3) i multi-monitor uživo potvrdu korisnika) |
| QM-3 | Glasovna integracija (WebSocket) | ✅ kod urađen (`desktop/ui/voice.py` RealtimeVoiceSession u QThread+asyncio, `tool_bridge.py` — tool execution kroz `POST /tools/execute` + confirmation bridge + idempotency, `confirmation_dialog.py` — armed min-delay, `voice_bus.py`; 34 desktop testa). 🟡 **End-to-end glas NIJE testiran** (headless — nema mikrofona/API ključa): stvarni tool-calling preko glasa, confirmation dijalog preko glasa i mjerenje latencije čekaju korisničko runtime testiranje. Ephemeral-credential-za-WebSocket protokol treba potvrdu na stvarnom ključu |
