...
| --- | --- | --- | --- |
| **Security Gate 0** (Electron hardening, IPC allowlist, backend localhost/auth, no arbitrary shell, tool manifest/risk model, active window validation, **execution_id/cancellation_token state mašina**, log redaction, path sandbox, security self-test MVP) | ✅ FAZA 3 (`core/ipc.cjs`), ✅ FAZA 4-5 (backend postoji, Electron ga pokreće — još bez auth tokena), ✅ FAZA 6 (realtime credential handling — standardni API ključ premješten na Python), ✅ FAZA 10 (permission engine + cancellation state mašina — `app/agent/permission_engine.py`, `app/agent/cancellation.py`, vidi `agent_reports/2026-07-05_faza10-permission-cancellation-engine.md`), ⬜ FAZA 11 (tool manifest primijenjen na stvarne migrirane toolove, active window validation, path sandbox) | **FAZA 13 i FAZA 14 (computer-use v1/v2)** — ne smiju se širiti van lokalnog dev prototipa dok Gate 0 nije zatvoren. Takođe blokira production installer (FAZA 19). | ⬜ Nije zatvoren — 4/5 stavki gotovo (IPC allowlist, backend postoji, realtime API key izolacija, permission/cancellation engine); preostaje backend local auth token (dio Security PR-1) i FAZA 11 (active window validation, path sandbox, primjena na stvarne migrirane toolove — permission engine iz FAZE 10 trenutno štiti samo Python-registrovane toolove, ne legacy PowerShell). Legacy PowerShell computer-use i dalje radi bez permission sloja direktno iz `electron/main.cjs` — vidi `SECURITY_MODEL.md` "Status implementacije" |
