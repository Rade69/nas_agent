...
| 8 | **Voice-first UI refactor** oko postojećeg `src/lib/realtime.ts` — TopBar/BottomVoiceBar, `VoiceState`, Realtime Event Router, Activity/transcript prikaz | ⬜ | Codex |
| 9 | **Confirmations + Plans/Proposals** — Approval dialog, `confirmation_id`, plan storage u SQLite (bez Notepad/auto-export) | ⬜ | Codex |
...
| **Security Gate 1** (document privacy modes, prompt injection boundaries, action receipt privacy status, audit trail, rate limits, dependency scanning, CI security checks) | FAZA 9 (confirmations/plans), buduća aktivacija Document Engine epic-a (vidi Backlog iznad), CI podešavanje | Beta/test korisnici | ⬜ Nije zatvoren |
