...
| **Voice Communication Reliability** (stabilizacija glasovne komunikacije — single-flight connect, cleanup, transport health, pouzdan event transport, fail-safe tool lifecycle, kontrolisan reconnect, VAD profili) | ✅ **R0 završen i verifikovan (pi + Codex, 2026-07-13)** + uski live-connect hotfix (Codex, 2026-07-13): `connect()` greška se više ne briše kroz `disconnect()`→`idle`; djelimični WebRTC resursi se čiste bez resetovanja error stanja; UI sada prikazuje stvarnu Realtime error poruku; Realtime model je prebačen na `gpt-realtime`. ✅ **R1 završen (pi + Codex review fix, 2026-07-14)**: single-flight `connect()` (paralelni pozivi ne stvaraju duple konekcije), connect timeout (30s, `_timeoutPromise` kroz DI), AbortController cancel/disconnect tokom povezivanja, timeout promise sada se eksplicitno završava na abort da `connect()` ne ostane pending ako neka neabortabilna setup operacija zapne, `connectionGeneration` guard protiv stale async callback-ova (DataChannel, PeerConnection, `handleServerEvent`, `executeFunctionCalls`), transport health (PeerConnection `onconnectionstatechange`, DataChannel `close`/`error`), error klasifikacija (insufficient_quota, billing, microphone, permission, unauthorized, timeout, fallback), 14 novih R1 testova (197 ukupno). R2/R3 (reconnect/backoff, serijski event queue) ostaju sljedeći paket. | FAZA 8 (voice-first UI), FAZA 6 (realtime session security) | [VOICE_COMMUNICATION_RELIABILITY_IMPLEMENTATION_PLAN_FOR_PI.md](./VOICE_COMMUNICATION_RELIABILITY_IMPLEMENTATION_PLAN_FOR_PI.md), [VOICE_COMMUNICATION_RELIABILITY_ANALYSIS_2026-07-13.md](./VOICE_COMMUNICATION_RELIABILITY_ANALYSIS_2026-07-13.md), [VOICE_COMMUNICATION_R1_BRIEF_FOR_PI.md](./VOICE_COMMUNICATION_R1_BRIEF_FOR_PI.md) |
