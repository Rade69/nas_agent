...
| **Voice Communication Reliability** (stabilizacija glasovne komunikacije — single-flight connect, cleanup, transport health, pouzdan event transport, fail-safe tool lifecycle, kontrolisan reconnect, VAD profili) | ✅ **R0 završen i verifikovan (pi + Codex, 2026-07-13)** — fail-closed diagnostics sanitizacija, 175 diagnostics testova + 8 DI/client testova (183 ukupno), DI seam u `realtime.ts`, minimalan Vitest runner, uklonjene nepotrebne zavisnosti (`@vitest/ui`, `jsdom`) i neiskorišteni DI članovi. Verifikacija: `test:voice`, `typecheck`, `build`, Python realtime testovi i `git diff --check` prolaze. R1 nije započet i traži novu eksplicitnu korisničku dozvolu. | FAZA 8 (voice-first UI), FAZA 6 (realtime session security) | [VOICE_COMMUNICATION_RELIABILITY_IMPLEMENTATION_PLAN_FOR_PI.md](./VOICE_COMMUNICATION_RELIABILITY_IMPLEMENTATION_PLAN_FOR_PI.md), [VOICE_COMMUNICATION_RELIABILITY_ANALYSIS_2026-07-13.md](./VOICE_COMMUNICATION_RELIABILITY_ANALYSIS_2026-07-13.md) |
