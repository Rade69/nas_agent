...
const toolSpecs = [
  ...
  },
  {
    ...
    name: "browser_tabs",
    risk: "medium",
    description: "List or activate tabs in an already-open Brave or Chrome window. Always call action=\"list\" FIRST — never guess tab numbers or titles. \"activate\" switches to an existing tab by its 1-based position in the most recent snapshot. The user's 'open the fifth tab' means ACTIVATE (switch to) tab #5, NOT create a new tab. After success, say exactly: 'Aktivirao sam N. tab: Title.' On TAB_SNAPSHOT_STALE, re-list and confirm the target. If BROWSER_EXTENSION_NOT_CONNECTED, tell the user and do NOT try keyboard shortcuts. Serbian 'Brejv' = Brave. To close a tab, use browser_tab_close instead.",
    parameters: {
      type: "object",
      properties: {
        ...
        position: { type: "number", minimum: 1, description: "1-based tab position in the snapshot. Required for activate." },
      },
      required: ["action"],
      additionalProperties: false,
    },
  },
  {
    ...
    risk: "high",
    description: "Close a Brave or Chrome browser tab. Requires user confirmation. Always call browser_tabs(action=\"list\") FIRST to get snapshot_id, profile_id, and positions. Pass profile_id from the snapshot to target the correct profile. On TAB_SNAPSHOT_STALE or TAB_PROFILE_MISMATCH, re-list and try again.",
    parameters: {
      type: "object",
      properties: {
        browser: { type: "string", enum: ["brave", "brejv", "chrome", "edge", "vivaldi", "opera", "opera_gx", "chromium"] },
