...
def test_invalid_model_no_silent_fallback():
    ...
    # (gpt-realtime, gpt-realtime-2) više nisu dozvoljeni.
    with pytest.raises(ValueError):
        resolve_openai_realtime_model("gpt-realtime")

    with pytest.raises(ValueError):
        resolve_openai_realtime_model("gpt-realtime-2")
...
def test_effective_user_choice_overrides_env():
    ...
    assert resolve_effective_realtime_model("gpt-realtime-2.1", "gpt-realtime-2.1-mini") == "gpt-realtime-2.1"


def test_effective_reverse_override():
    # T-B5: env 2.1, korisnik izabrao 2 → 2.
    assert (
        resolve_effective_realtime_model("gpt-realtime-2.1-mini", "gpt-realtime-2.1")
