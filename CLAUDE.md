...
- **File header komentar.** Svaki kod fajl (`.py`, `.ts`, `.tsx`, `.cjs`, `.css`) koji se kreira ili značajno mijenja mora imati kratak komentar/docstring na samom vrhu: šta fajl radi, koji problem rješava, i (ako nije očigledno iz naziva/putanje) kako se uklapa u ostatak sistema. 2-5 linija, ne esej — cilj je da se skeniranjem vrha fajla odluči da li je to pravo mjesto za traženi podatak, bez čitanja cijelog sadržaja. Format: Python `"""..."""` docstring odmah ispod eventualnog `from __future__ import annotations`; TS/TSX `/** ... */` na samom vrhu fajla (isti obrazac kao postojeći `DictationScreen.tsx`, `PixelMockupBoard.tsx`); `.cjs` `/** ... */` ili `//` blok na vrhu; `.css` `/* ... */` blok na vrhu (isti obrazac kao postojeći `11-pixel-shell.css`, `07-companion-orb.css`). Postojeći fajlovi bez header-a se ne moraju hitno dopunjavati — pokrivaju se organski kad se dirne dati fajl, ili kroz namjenski retroaktivni prolaz (vidi `docs/PI_TASK_FILE_HEADER_COMMENTS_BRIEF.md`).
...
This project is indexed by GitNexus as **nas_agent** (6640 symbols, 10186 relationships, 172 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
